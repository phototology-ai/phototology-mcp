...
function buildServerInstructions(): string {
  ...
  return [
    ...
    `${lenses}. Each owns a specific set of top-level output fields. Pass a subset via \`modules: [...]\` to bill only for what you need.`,
    '',
    '## Presets',
    `Bundled lens sets: ${presets}. Use a preset when the workflow matches (e.g. \`memorial\` for tribute photos, \`automobile\` for vehicle analysis). Prefer explicit \`modules\` for precise billing.`,
    '',
    '## How to be cheap',
