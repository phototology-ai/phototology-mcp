  ...
  s.registerTool(
    'get_credits',
    {
      description: [
        ...
        '',
        'Returns: `{ tier, community: { balance, monthlyAllowance, resetsInDays, referralBonus? }, purchased: { balance }, reserved }`. Phototology uses a dual-pool model: every account gets 1,000 community credits per month (free, auto-reset, no rollover) plus any purchased packs (1 credit = $0.01). Effective spendable credits = `community.balance + purchased.balance - reserved`. Community credits are spent FIRST, paid credits SECOND.',
        '',
